...
class RpcClientProcessService:
    ...
    def doCoreRpc(sourceNodeId, rpcId, reqId, contentByteString, timestamp):
        if rpcId == RpcId.UNKNOWN_RPC_ID:
            ...
            try:
                ...
                RpcClientProcessService.checkCommonRtn(rpcOrderRtn.commonRtn, sourceNodeId)
            ...
            try:
                ...
                RpcClientProcessService.checkCommonRtn(rpcTradeRtn.commonRtn, sourceNodeId)
            ...
            try:
                ...
                RpcClientProcessService.checkCommonRtn(rpcPositionRtn.commonRtn, sourceNodeId)
            ...
            try:
                ...
                RpcClientProcessService.checkCommonRtn(rpcAccountRtn.commonRtn, sourceNodeId)
            ...
            try:
                ...
                RpcClientProcessService.checkCommonRtn(rpcContractRtn.commonRtn, sourceNodeId)
            ...
            try:
                ...
                RpcClientProcessService.checkCommonRtn(rpcTickRtn.commonRtn, sourceNodeId)
            ...
            try:
                ...
                RpcClientProcessService.checkCommonRtn(rpcNoticeRtn.commonRtn, sourceNodeId)
    ...
    @staticmethod
    def checkCommonRtn(commonRtn, sourceNodeId):
        if not commonRtn:
            logger.error("参数commonRtn缺失")
            raise Exception("参数commonRtn缺失")

        if sourceNodeId != commonRtn.sourceNodeId:
            logger.error("不合法的来源节点ID")
            raise Exception("不合法的来源节点ID:" + commonRtn.sourceNodeId)

        if commonRtn.targetNodeId != Config.nodeId:
            raise Exception("目标节点ID不为" + Config.nodeId + "!")
    ...
    def checkCommonRsp(commonRsp, sourceNodeId, reqId):
        ...
        if sourceNodeId != commonRsp.sourceNodeId:
            logger.error("不合法的来源节点ID")
            raise Exception("不合法的来源节点ID:" + commonRsp.sourceNodeId)

        if commonRsp.targetNodeId != Config.nodeId:
            raise Exception("目标节点ID不为" + Config.nodeId + "!")
